...
import scripts.actions.UseItemWithAnother;
import scripts.actions.Withdraw;
import scripts.actions.Cut.Cut;
import scripts.actions.Cut.EnterAmountToCut;
import scripts.actions.Cut.SelectMakeX;
import scripts.actions.DeathWalking.DeathWalk;
import scripts.actions.Make.Make10Sets;
import scripts.actions.Make.MakeUnf;
import scripts.actions.StringBow.SelectMakeAll;
import scripts.actions.StringBow.StringBow;
...
public class MFFletcherGUI extends AbstractGUI {
	...
	public List<State> createStateAssociations() {
		...
		if (makeRadioButton.isSelected()) {
			making.addActions(new UseItemWithAnother(), new Make10Sets(), new MakeUnf());
			states.add(making);
		} else if (fletchRadioButton.isSelected()) {
			cutting.addActions(new UseItemWithAnother(),new SelectMakeX(), new EnterAmountToCut(), new Cut());
			states.add(cutting);
		} else {
			stringing.addActions(new UseItemWithAnother(), new SelectMakeAll(),new StringBow());
	...
	private void processActions() {
		if (makeRadioButton.isSelected()) {
			...
			skillManager.loadMasterIndex(582);
			...
			skillManager.loadMasterIndex(309);
		}
	}
